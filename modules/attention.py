...
class MultiSelfAttention(nn.Module):
    def __init__(self,drouout : float= None,casual : bool = False):
        ...
        if drouout is not None:
            self.dropout = nn.Dropout(p = drouout)
        #TODO
    def forward(self,x,pad_mask = None):
        ...
        if pad_mask is not None:
            ...
            att = att.masked_fill(pad_mask.unsqueeze(1).unsqueeze(2) == 0,float('-inf'))
...
class MultiCrossAttention(nn.Module):
    ...
    def forward(self,x,pad_mask = None,memory = None):
        """_summary_
        ...
        B,T,C = x.shape
        hs = C//self.n_head
        assert memory is not None, "Memory must be provided for cross attention"
        assert C % self.n_head == 0, f"assertion error number of heads must be a factor of embedding dim. {C} % {self.n_head} != 0"
        
        q = self.cross_q(x) #shape (B,T,C)
        
        print(f"B={B}, T={T}, n_head={self.n_head}, hs={hs}, x.shape={x.shape}, q.shape={q.shape}")
